...
#define REG_PRECOMP 1    /* start of write precompensation 预补偿 */
#define REG_COUNT   2    /* sectors部门 to transfer */
...
#define CMD_IDLE        0x00  /* for w_command: drive idle 驱动空闲 */
#define CMD_RECALIBRATE 0x10  /* recalibrate drive 重新校准驱动器 */
...
#define CMD_READVERIFY  0x40  /* read verify 读入验证 */
#define CMD_FORMAT      0x50  /* format track 格式轨道 */
#define CMD_SEEK        0x70  /* seek cylinder */
#define CMD_DIAG        0x90  /* execute device diagnostics 执行设备诊断 */
#define CMD_SPECIFY     0x91  /* specify parameters 指定参数 */
#define ATA_IDENTIFY    0xEC  /* identify drive */
/* #define REG_CTL 0x206 */   /* control register */
#define REG_CTL         0     /* control register */
#define CTL_NORETRY     0x80  /* disable access retry 禁用访问重试 */
#define CTL_NOECC       0x40  /* disable ecc retry 禁用ecc重试 */
...
#define STATUS_DMADF    0x20  /* dma ready/drive fault 准备/驱动器故障 */
                              /* 直接存储器访问（Direct Memory Access，DMA） */
#define STATUS_SRVCDSC  0x10  /* service or dsc */
                              /* Desired State Configuration (DSC) */
#define STATUS_DRQ      0x08  /* data transfer request */
#define STATUS_CORR     0x04  /* correctable error occurred 发生可纠正错误 */
#define STATUS_CHECK    0x01  /* check error 检查错误 */
...
// at_winchester_task        在_温彻斯特_任务
// init_params           初始化_参数
// init_drive            初始化_驱动
// init_params_pci       初始化_参数_Peripheral Component Interconnect(外设部件互连标准)
// w_do_open           温彻斯特_做_打开
// w_prepare           温彻斯特_准备
// w_identify          温彻斯特_标识
// w_name              温彻斯特_姓名
// w_io_test           温彻斯特_输入输出_测试
// w_specify           温彻斯特_详述
// do_transfer               做_转移
// w_transfer          温彻斯特_转移
// com_out                 命令_输出
// w_need_reset        温彻斯特_需要_重新设置
// w_do_close          温彻斯特_做_关闭
// com_simple              命令_简单
// w_timeout           温彻斯特_超时
// w_reset             温彻斯特_重新设置
// w_intr_wait         温彻斯特_中断_等待
// at_intr_wait              在_中断_等待
// w_waitfor           温彻斯特_等待为了
// w_geometry          温彻斯特_几何
// w_other             温彻斯特_其他
// w_hw_int            温彻斯特_硬件_整型
...
*/
    ...
    if (com_simple(&cmd) == OK) {
        ...
        for (i = 0; i < 40; i++) w_id_string[i] = id_byte(27)[iˆ1];
    ...
    while (nr_req > 0) {
        ...
        while (r == OK && nbytes > 0) {
            ...
            */
            ...
            if (opcode == DEV_GATHER) { if((s=sys_insw(wn->base_cmd+REG_DATA,
                proc_nr,(void*)iov->iov_addr,SECTOR_SIZE))!=OK)
                    panic(w_name(),"Call to sys_insw() failed", s);
