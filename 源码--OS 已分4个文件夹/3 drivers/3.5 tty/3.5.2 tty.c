...
// tty_icancel    电传机_i取消
// tty_init       电传机_初始化
// tty_timed_out  电传机_时间_输出
// expire_timers    到期_定时器
// settimer          设置定时器
// tty_devnop     电传机_设备没有任何
// do_select          做_选择
...
*/
    ...
    for (ct = 0; ct < count; ct++) {
        ...
        if (tp->tty_termios.c_lflag & IEXTEN) {
            ...
            /* LNEXT (ˆV) to escape the next character? */
            ...
            /* REPRINT (ˆR) to reprint echoed characters? */
        ...
        if (tp->tty_termios.c_lflag & ICANON) {
            ...
            /* EOF (ˆD) means end-of-file, an invisible "line break". */
        ...
        if (tp->tty_termios.c_iflag & IXON) {
            ...
            /* Output restarts on START (ˆQ) or any character if IXANY. */
            if (tp->tty_inhibited) {
        ...
        /* Check for INTR (ˆ?) and QUIT (ˆ\) characters. */
...
* with their normal effect, other control characters are echoed as "ˆX",
* normal characters are echoed normally. EOF (ˆD) is echoed, but immediately
...
* messed up by output, or if REPRINT (ˆR) is typed.
*/
    int count;
    u16_t *head;
    ...
    /* Show REPRINT (ˆR) and move to a new line. */
