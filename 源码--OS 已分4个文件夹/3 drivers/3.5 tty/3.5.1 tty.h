...
char tty_escaped;     /* 1 when LNEXT (ˆV) just seen, else 0 */
char tty_inhibited;   /* 1 when STOP (ˆS) just seen (stops output) */
...
#define NOT_ESCAPED 0  /* previous character is not LNEXT (ˆV) */
#define ESCAPED 1      /* previous character was LNEXT (ˆV) */
#define RUNNING 0      /* no STOP (ˆS) has been typed to stop output */
#define STOPPED 1      /* STOP (ˆS) has been typed to stop output */
...
#define IN_EOT 0x1000  /* char is a line break (ˆD, LF) */
#define IN_EOF 0x2000  /* char is EOF (ˆD), do not return to user */
#define IN_ESC 0x4000  /* escaped by LNEXT (ˆV), no interpretation */
