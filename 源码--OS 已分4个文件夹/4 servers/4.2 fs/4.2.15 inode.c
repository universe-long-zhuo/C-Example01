...
*/
    ...
    register struct inode *rip, *xp;  /* rip、xp是什么？ */

    /* Search the inode table both for (dev, numb) and a free slot. 空闲存储区 */
    xp = NIL_INODE;
...
{
/* The V1.x IBM disk, the V1.x  disk, and the V2 disk (same for IBM and
* ) all have different inode layouts. When an inode is read or written
...
/* Same as old_icopy, but to/from V2 disk layout. */
    ...
    if (direction == READING) {
        ...
        vdip->d2_atime = conv4(norm,rip->i_atime);
